...
cat > spark_driver_pod_template_without_karpenter.yml <<EOF 
apiVersion: v1
kind: Pod
spec:
...
cat > hp_spark_executor_pod_template_without_karpenter.yml <<EOF 
apiVersion: v1
kind: Pod
spec:
...
cat > lp_spark_executor_pod_template_without_karpenter.yml <<EOF 
apiVersion: v1
kind: Pod
spec:
...
aws s3 cp spark_driver_pod_template_without_karpenter.yml $POD_TEMPLATE_PATH
aws s3 cp hp_spark_executor_pod_template_without_karpenter.yml $POD_TEMPLATE_PATH
aws s3 cp lp_spark_executor_pod_template_without_karpenter.yml $POD_TEMPLATE_PATH
