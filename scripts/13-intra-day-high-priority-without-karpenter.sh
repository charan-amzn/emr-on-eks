...
# Job Submission
export CURRENT_VIRTUAL_CLUSTER=intra-day-submission-emr-cluster
...
--execution-role-arn "$(aws iam get-role --role-name intra-day-submission-JobExecutionRole --query Role.Arn --output text)" \
--release-label emr-6.4.0-latest \
--job-driver '{
    "sparkSubmitJobDriver": {
        "entryPoint": "'${DEMO_JOBS_PATH}'/high-priority-threadsleeper.py",
        "sparkSubmitParameters": "--conf spark.kubernetes.driver.podTemplateFile=\"'${POD_TEMPLATE_PATH}'/spark_driver_pod_template_without_karpenter.yml\" --conf spark.kubernetes.executor.podTemplateFile=\"'${POD_TEMPLATE_PATH}'/hp_spark_executor_pod_template_without_karpenter.yml\" --conf spark.executor.instances=2 --conf spark.executor.memory=2G --conf spark.executor.cores=1 --conf spark.driver.cores=1"
        }
}' \
--configuration-overrides='{
    ...
    "monitoringConfiguration": {
            ...
            "cloudWatchMonitoringConfiguration": {
                "logGroupName": "/emr-on-eks/'${CLUSTER_NAME}'/intra-day-submission",
                "logStreamNamePrefix": "high-p-"
            }, 
            "s3MonitoringConfiguration": { 
            "logUri": "s3://intra-day-submission-logs-'${AWS_ACCOUNT_ID}-${AWS_REGION}'/" 
            }
